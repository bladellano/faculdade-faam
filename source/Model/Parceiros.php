...
class Parceiros extends Model
{
    ...
    public static function listAll(array $conditions = [], $fields = "*", $operator = ' AND ', $limit = "")
    {
        ...
        try {
            ...
            $SQL = "SELECT {$fields} FROM parceiros WHERE TRUE ";
    ...
    {
        $sql = new Sql();
        $results = $sql->select("SELECT * FROM parceiros WHERE slug = :slug", ['slug' => $slug]);
    ...
    public function save()
    {
        ...
        $result = $sql->{$action}("parceiros", $this->getValues());
    ...
    {
        $sql = new Sql();
        $results = $sql->select("SELECT * FROM parceiros WHERE id = :id", [":id" => $id]);
        $this->setData($results[0]);
    }
    ...
    {
        $sql = new Sql();
        $sql->query('DELETE FROM parceiros WHERE id = :id', [":id" => $this->getid()]);
    }
